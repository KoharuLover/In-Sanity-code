...
Input:
	...
	//this is where players need to go find the key (Go Living Room, Go Master Bedroom, Kitchen)
...
void Story::EncounterHUD()
{
	...
	std::cout << "| Health: " << Austin.getSanity() << "            | Health: " << Creature.getENHP() << std::endl;
	std::cout << "| Damage: " << Austin.getAttack() << std::endl;
	std::cout << "---------------------------------------------------------------\n";
}
	...
	while (true) {
		...
		if (GetAsyncKeyState(VK_SPACE) & 0x8000) {
			...
			if (spacePressed) {
				...
				std::string closet3 = "He forces the door open and grabs you from inside the closet. Darkness envelops you once more. Close. Not close enough.";
	...
	while (true) {
		...
		if (GetAsyncKeyState(VK_SPACE) & 0x8000) {
			...
			if (spacePressed) {
				...
				held = false;
				// Handle case where spacebar was released too early
				std::string bed4 = "He hears your hitched breathing underneath the bed. Crouching Down, He grabs you by the throat. \nDarkness envelops you once more. Close. Not close enough.";
